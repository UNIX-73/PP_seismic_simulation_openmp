...
void abe_matrix(double vertices[][3], int bv[], struct properties *prop,
				double Ce[]);
...
int main(int argc, char **argv)
{
	...
	*/
	...
	if (!options.quiet) fprintf(stderr, "%s: Beginning simulation.\n", argv[0]);
...
#pragma omp parallel for schedule(static)
	...
	if (Src.epicenternode != 0 && Src.epicenternode <= ARCHmine) {
		fprintf(
			stderr, "The epicenter is node %d at (%f  %f  %f)\n",
	...
	if (Src.sourcenode != 0) {
		for (i = 0; i < ARCHelems; i++) {
			...
			if (cor[0] == Src.sourcenode || cor[1] == Src.sourcenode ||
				...
				source_elms[i] = 2;
				if (point2fault(xc) >= 0) source_elms[i] = 3;
			}
		}
	}
	...
	for (i = 0; i < ARCHelems; i++) {
		...
		verticesonbnd = 0;
		for (j = 0; j < 4; j++)
			if (nodekind[cor[j]] != 1) bv[verticesonbnd++] = j;
		...
		*/
		...
		for (j = 0; j < 4; j++)
			for (k = 0; k < 3; k++) vertices[j][k] = ARCHcoord[cor[j]][k];
		...
		for (j = 0; j < 12; j++) Ce[j] = Ce[j] + alpha * Me[j];
...
#pragma omp for
		...
		i = disptminus;
		disptminus = dispt;
		dispt = disptplus; /*Dependencia entre iteraciones, lo que se escribe en
							  una iteración se lee en la siguiente*/
		disptplus = i;
	}
...
void shape_ders(ds) double ds[][4];
{
	ds[0][0] = -1;
	ds[1][0] = -1;
	ds[2][0] = -1;
...
void get_Enu(prop, E, nu) struct properties *prop;
double *E, *nu;
{
	double ratio;
...
void element_matrices(vertices, prop, Ke, Me) double vertices[][3], Ke[][12],
	Me[];
struct properties *prop;
{
	double ds[3][4];
	...
	for (i = 0; i < 3; i++)
		for (j = 0; j < 3; j++) {
			sum[0] = 0.0;
			for (m = 0; m < 4; m++) sum[0] = sum[0] + ds[i][m] * vertices[m][j];
	...
	for (m = 0; m < 4; m++) {
		for (i = 0; i < 3; i++) {
			sum[i] = 0.0;
			for (j = 0; j < 3; j++) sum[i] = sum[i] + jacobian[j][i] * ds[j][m];
		}

		for (i = 0; i < 3; i++) ds[i][m] = sum[i];
	}
		...
		for (i = 0; i < 3; ++i) {
			...
			for (n = 0; n <= m; n++) {
				for (j = 0; j < 3; j++) {
					...
					if (i == j) {
						ts = ts * c1;
						tt = (ds[0][m] * ds[0][n] + ds[1][m] * ds[1][n] +
							  ds[2][m] * ds[2][n]) *
							 c3;
					} else {
						if (m == n) {
	...
	for (i = 0; i < 12; i++) Me[i] = tt;

	for (i = 0; i < 12; i++)
		for (j = 0; j <= i; j++) Ke[j][i] = Ke[i][j];
}
/* --------------------------------------------------------------------------*/
...
void abe_matrix(vertices, bv, prop, Ce) double vertices[][3];
int bv[];
struct properties *prop;
double Ce[];
{
...
void centroid(x, xc) double x[][3], xc[];
{
	int i;
...
void mv12x12(m, v) double m[][12], v[];
{
	int i, j;
	double u[12];
...
void vv12x12(v1, v2, u) double v1[], v2[], u[];
{
	int i;

	for (i = 0; i < 12; i++) u[i] = v1[i] * v2[i];
}

/*--------------------------------------------------------------------------*/
/* Graceful exit                                                            */

void arch_bail(void) { exit(0); }
/*--------------------------------------------------------------------------*/
...
void arch_info(void)
{
	...
	printf(
		"    -Q  Quietly suppress all explanation of what this program is "
		"doing\n");
...
void readpackfile(FILE *packfile, struct options *op)
{
	...
	/* read elements */
	if (!op->quiet) fprintf(stderr, "%s: Reading elements.\n", progname);
...
void smvp_opt(nodes, A, Acol, Aindex, v, w) int nodes;
double ***A;
int *Acol;
int *Aindex;
double **v;
double **w;
{
	...
	for (i = 0; i < nodes; i++) {
		...
		Anext = Aindex[i];
		Alast = Aindex[i + 1];
		sum0 = w[i][0] + A[Anext][0][0] * vi0 + A[Anext][0][1] * vi1 +
			   A[Anext][0][2] * vi2;
		sum1 = w[i][1] + A[Anext][1][0] * vi0 + A[Anext][1][1] * vi1 +
			   A[Anext][1][2] * vi2;
		sum2 = w[i][2] + A[Anext][2][0] * vi0 + A[Anext][2][1] * vi1 +
			   A[Anext][2][2] * vi2;
		Anext++;
		while (Anext < Alast) {
			col = Acol[Anext];
